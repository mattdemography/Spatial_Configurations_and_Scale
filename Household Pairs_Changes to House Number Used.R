...
for(i in 1:rows){
  tryCatch({
    ...
    #This is the dissolved ExtSeg_Join2
      ...
      #Order by Address and House Number
        Segment$nhn<-trim(laply(strsplit(as.character(Segment$match_addr), split=" "), "[", 1))
        Segment<-Segment[order(Segment$newaddress,Segment$nhn),]
      #Tag Unique Streets and Housenumbers - I.E. BUILDINGS
        Segment$stnum_id<-id(Segment[c("newaddress","nhn")], drop=T)
    ...
    merge2<-merge(x=merge1,y=Segment[,c("serial","nhn","newaddress","stnum_id", "side","type", "segside")], 
    ...
    merge2<-merge2[complete.cases(merge2$side,merge2$nhn),]
    ...
    #########Neighbors by Race ##########
      merge3$stnum_id<-as.numeric(merge3$stnum_id)
    ...
    #Order
      sgright<-sgright[order(sgright$newaddress, sgright$nhn),]
      sgleft<-sgleft[order(sgleft$newaddress, sgleft$nhn),]
  ...
  write.csv(AllSegTable, file="C:/Users/mmarti24/Dropbox/Papers/SIS - Early Arriving/Southern Cities_Change in House Number.csv",row.names=F)
