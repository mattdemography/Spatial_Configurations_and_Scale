#Library List
  ...
  sumby_1<- function(x,y){
    ...
    for(i in 1:nrows){
      ...
      t<-(tapply(x[,i], INDEX=list(y), FUN=sum, na.rm=T))
      df2<-data.frame(x=names(t), y=t)
      df2$y<-ifelse(df2$y>0, df2$y-1, df2$y)
      df2<-rename(df2, c(x=y2, y=x2))
      df<-merge(df, df2, by=y2, all=T, accumulate=T)
      df<-df[!names(df) %in% myvars]
      
      #Add Back Person to Total unless 
    }
    df
  }
...
#########Neighbors by Race ##########
        ...
        build_1<-sumby_1(All[,c("german", "irish", "native_white", "Person")], All$Address)
        build_1<-rename(build_1, c("german"="g_build_1", "irish"="i_build_1", "Person"="t_build_1"))
        
        build<-merge(y=build, x=build_1, by="Address", all.x=T)
        ...
        build<-subset(build, build$t_build_1!=0)
        
        build$IsoBLD_g<-((build$g_build/(build$german_t))*(build$g_build_1/build$t_build_1))
        SegTable$IsoBLD_g<-sum(build$IsoBLD_g)
        build$IsoBLD_i<-((build$i_build/(build$irish_t))*(build$i_build_1/build$t_build_1))
        SegTable$IsoBLD_i<-sum(build$IsoBLD_i)
        ...
        #Drop Cases where person = NA.  This would occur if a building were geocoded on two segments.
          ...
          #Sum by Neighbor
          BLD<-sumby_1(hhfinal[,c("Ntotg", "Ntoti", "Ntotw", "Ntott")], hhfinal$Person)
      ...
      #Calculate Exposure - # WEIGHT IS BUILDING (E_build/ETHNICITY_t)
        ...
        #First we must recalculate building totals because we now want to subtract the focal household from the calculations
        #We do not have to recalculate native whites as their numbers should not eliminate the focal household.
        build<-sumby_1(All[,c("german", "irish", "Person")], All$Address)
        build<-rename(build, c("german"="g_build", "irish"="i_build", "Person"="t_build"))
        build<-subset(build, !duplicated(build$Address))
        build<-subset(build, !is.na(build$g_build))
        
        myvars<-c("g_build", "i_build", "t_build")
        hhfinal<-hhfinal[!names(hhfinal) %in% myvars]
        hhfinal<-merge(x=hhfinal, y=build, by="Address")
        hhfinal<-subset(hhfinal, hhfinal$t_build!=0)
        
      #At Building Level
        hhfinal$IsoBLD_g<-((hhfinal$g_build/(hhfinal$german_t))*(hhfinal$g_build/hhfinal$t_build))
        SegTable$IsoBLD_g<-sum(hhfinal$IsoBLD_g)
        hhfinal$IsoBLD_i<-((hhfinal$i_build/(hhfinal$irish_t-1))*(hhfinal$i_build/hhfinal$t_build))
        SegTable$IsoBLD_i<-sum(hhfinal$IsoBLD_i)
        
      #Dissimilarity at Building Level
        hhfinal$DissimBLD_g<-abs((hhfinal$g_build/(hhfinal$german_t-1))-(hhfinal$w_build/hhfinal$native_white_t))
        SegTable$DissimBLD_g<-0.5*(sum(hhfinal$DissimBLD_g))
        hhfinal$DissimBLD_i<-abs((hhfinal$i_build/(hhfinal$irish_t-1))-(hhfinal$w_build/hhfinal$native_white_t))
        SegTable$DissimBLD_i<-0.5*(sum(hhfinal$DissimBLD_i))
          ...
          #Do Calculations
            MultiUnit<-sumby_1(multi[,c("german", "irish", "Person")], multi$Address)
            MultiUnit<-rename(MultiUnit, c("german"="g_multi","irish"="i_multi", "Person"= "t_multi"))
        ...
        #Isolation
          multi$IsoMultiBLD_g<-((multi$g_multi/(multi$german_t-1))*(multi$g_multi/multi$t_multi))
          SegTable$IsoMultiBLD_g<-sum(multi$IsoMultiBLD_g)
          multi$IsoMultiBLD_i<-((multi$i_multi/(multi$irish_t-1))*(multi$i_multi/multi$t_multi))
      ...
      #At Building Group Level
        #Isolation
          hhfinal$IsoBLDGroup_g<-((hhfinal$g_build/(hhfinal$german_t-1))*(hhfinal$Ntotg/hhfinal$Ntott))
          SegTable$IsoBLDGroup_g<-sum(hhfinal$IsoBLDGroup_g, na.rm=T)
          hhfinal$IsoBLDGroup_i<-((hhfinal$i_build/(hhfinal$irish_t-1))*(hhfinal$Ntoti/hhfinal$Ntott))
          SegTable$IsoBLDGroup_i<-sum(hhfinal$IsoBLDGroup_i, na.rm=T)
        
      #At Building Level - SANS YOUR BUILDING
        #Isolation
        hhfinal2$IsoBLDGroup2_g<-((hhfinal2$g_build/(hhfinal2$german_t-1))*(hhfinal2$Ntotg2/hhfinal2$Ntott2))
        SegTable$IsoBLDGroup2_g<-sum(hhfinal2$IsoBLDGroup2_g, na.rm=T)
        hhfinal2$IsoBLDGroup2_i<-((hhfinal2$i_build/(hhfinal2$irish_t-1))*(hhfinal2$Ntoti2/hhfinal2$Ntott2))
        SegTable$IsoBLDGroup2_i<-sum(hhfinal2$IsoBLDGroup2_i, na.rm=T)
      
      ###### Segment Side #####
        Segside<-sumby_1(All[,c("german", "irish", "Person")], All$segside)
        Segside<-rename(Segside, c("german"="g_side", "irish"="i_side", "Person"="t_side"))
        ...
        Segside<-subset(Segside, Segside$t_side!=0)
        
        #ISOLATION
        Segside$IssoSegside_g<-((Segside$g_side/(Segside$german_t-1))*(Segside$g_side/Segside$t_side))
        SegTable$IsoSegside_g<-sum(Segside$IssoSegside_g)
        Segside$IssoSegside_i<-((Segside$i_side/(Segside$irish_t-1))*(Segside$i_side/Segside$t_side))
      ...
      ##### Segment #####
        Segment<-sumby_1(All[,c("german", "irish", "Person")], All$segment_id)
        Segment<-rename(Segment, c("german"="g_seg", "irish"="i_seg", "Person"="t_seg"))
        ...
        Segment<-subset(Segment, Segment$t_seg!=0)
        
        #ISOLATION
        Segment$IssoSeg_g<-((Segment$g_seg/(Segment$german_t-1))*(Segment$g_seg/Segment$t_seg))
        SegTable$IsoSeg_g<-sum(Segment$IssoSeg_g)
        Segment$IssoSeg_i<-((Segment$i_seg/(Segment$irish_t-1))*(Segment$i_seg/Segment$t_seg))
  ...
  write.csv(AllSegTable, "C:/Users/mmarti24/Dropbox/Papers/SIS - Early Arriving/Segregation_Results_Minus1.csv")
  write.xlsx(AllSegTable, file="C:/Users/mmarti24/Dropbox/Papers/SIS - Early Arriving/Segregation_Results_Minus1.xlsx")
