...
for(i in 1:40){
  tryCatch({
    ...
    #Germans (_f=first generation; _s=second generation; _t=total)
      ...
      All$german_all<-ifelse(All$german_f==1 | All$german_s==1, 1, 0)
    ...
    #Irish (_f=first generation; _s=second generation; _t=total)
      ...
      #######Make Change to German Definition Here #####
      All$irish_all<-ifelse(All$irish_f==1 | All$irish_s==1, 1, 0)
      All$irish<-ifelse(All$irish_f==1, 1, 0)
    #Keep only adults      
      All<-All[which(All$Above18==1),]
      
      t<-data.frame(table(All$german_all))
      All$german_t<-t[2,2]
      
      t<-data.frame(table(All$irish_all))
      All$irish_t<-t[2,2]
...
#########Neighbors by Race ##########
  ...
  write.csv(AllSegTable, "C:/Users/mmarti24/Dropbox/Papers/SIS - Early Arriving/Segregation_Results_1stGen_1st2ndExp.csv")
  write.xlsx(AllSegTable, file="C:/Users/mmarti24/Dropbox/Papers/SIS - Early Arriving/Segregation_Results_1stGen_1st2ndExp.xlsx")
